  ...
  window.CotBRuntime.installMonsterAi = function (context) {
    with (context) {
      ...
      function monsterCanMoveTo(monster, x, y) {
        return !solidAt(x, y) && !monsterAt(x, y) && (x !== state.x || y !== state.y) && monsterCanEnterTerrain(monster, terrainAt(x, y));
      }
      ...
      function hurtLiveMember(basePower, spread, element = null) {
        ...
        const baseDamage = Math.max(1, Math.round(basePower + Math.random() * spread - memberDefense(defender)));
        const damage = partyElementDamage(baseDamage, element);
      ...
      function monsterMelee(monster, messages) {
        ...
        for (const attack of attacks) {
          const hit = hurtLiveMember(attack.damage / 3 + ((monster.mightTurns || 0) > 0 ? 2 : 0) + ((monster.rageTurns || 0) > 0 ? 2 : 0), 3);
          if (!hit || !hit.defender) return true;
          addEffect("impact", [{ x: state.x, y: state.y }]);
          messages.push(`${monster.name} ${attackVerb(attack.type)} ${hit.defender.name} for ${hit.damage}.${applyMonsterTraits(monster, attack, hit.defender)}`);
          if (!liveMember()) {
            state.defeated = true;
      ...
      function runMonsterTurns(floorState, messages) {
        for (const monster of floorState.monsters) {
          ...
          const monsterSpeed = (monster.speed || 10) + ((monster.hasteTurns || 0) > 0 ? 5 : 0) + ((monster.rageTurns || 0) > 0 ? 5 : 0);
